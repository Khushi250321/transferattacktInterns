...
def mi_fgsm(model, x, tgt_emb, attack_type):
    ...
    for _ in range(NUM_ITER):
        ...
        adv = tf.clip_by_value(adv, -1.0, 1.0)
    return adv
def pgn_attack(model, x, tgt_emb, attack_type, num_neighbors=10, zeta=3.0, beta=1.5):
    """
    PGN: Penalizing Gradient Norm (NeurIPS 2023)
    Adversarial examples in flat regions of the loss surface transfer
    better across models. PGN approximates flatness cheaply by sampling
    a nearby point, interpolating its gradient with the current gradient
    (approximating the Hessian/vector product), and averaging over
    several random neighbors to get a stable, flat-region direction.
    """
    adv = tf.identity(x)
    g = tf.zeros_like(x)
    alpha = EPSILON / NUM_ITER
    ...
    def grad_at(point):
        with tf.GradientTape() as tape:
            tape.watch(point)
            emb = compute_embedding(model, point)
            cos = tf.reduce_sum(emb * tgt_emb, axis=1)
            loss = attack_loss(cos, attack_type)
        return tape.gradient(loss, point)

    for _ in range(NUM_ITER):
        grad_x = grad_at(adv)
        avg_grad = tf.zeros_like(x)
        for _ in range(num_neighbors):
            noise = tf.random.uniform(tf.shape(x), -1.0, 1.0)
            x_neighbor = adv + zeta * EPSILON * tf.sign(noise)
            x_neighbor = tf.clip_by_value(x_neighbor, x - EPSILON, x + EPSILON)
            x_neighbor = tf.clip_by_value(x_neighbor, -1.0, 1.0)
            grad_neighbor = grad_at(x_neighbor)
            combined = grad_x + beta * (grad_neighbor - grad_x)
            avg_grad += combined
        avg_grad = avg_grad / num_neighbors
...
def vmi_fgsm(model, x, tgt_emb, attack_type, beta=1.5, n=20):
   ...
   CVPR 2021 - Wang et al.
    ...
    for _ in range(NUM_ITER):
        ...
        # Compute variance of gradients in neighborhood
        ...
        for _ in range(n):
            ...
            with tf.GradientTape() as tape2:
                tape2.watch(neighbor)
                emb2 = compute_embedding(model, neighbor)
                cos2 = tf.reduce_sum(emb2 * tgt_emb, axis=1)
                loss2 = attack_loss(cos2, attack_type)
            g2 = tape2.gradient(loss2, neighbor)
            grad_var += g2 - grad
        v = grad_var / n

        # Momentum update with variance tuning
        grad_tuned = grad + v
        grad_tuned = grad_tuned / (tf.reduce_mean(tf.abs(grad_tuned)) + 1e-8)
        g = DECAY * g + grad_tuned
        adv = adv + alpha * tf.sign(g)
        adv = tf.clip_by_value(adv, x - EPSILON, x + EPSILON)
        adv = tf.clip_by_value(adv, -1.0, 1.0)
